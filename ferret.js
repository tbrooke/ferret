/******************************************************************************
Original Copyright 2013, Frank Bennett

https://bitbucket.org/fbennett/free-law-ferret/src/223f6ce8c7c8612b00c1c4de33f237e8966f4dd6/chrome/content/ferret.js?at=default
Modified 2014, Matthew Zorn
********************************************************************************/
...
(function() {
  ...
  var _utils = {
    keys: function(obj) {
      ...
      var keys = [];
      for (var key in obj) if (_utils.has(obj, key)) keys.push(key);
      return keys;
    },
    values: function(obj) {
      var keys = _utils.keys(obj);
  ...
  Citation.prototype.canonical_citation = function () {
      var rep = Ferret.constants.VARIATIONS_ONLY[this.reporter] || this.reporter;
      var volume = this.volume ? this.volume + " " : "";
      return volume + rep + " " + this.page;
  }

  Citation.prototype.as_regex = function (options, variations) {

      var rep = (typeof variations === 'undefined') ?
                  this.reporter :
                  "(" + variations.concat(this.reporter).join("|") + ")";
      console.log(rep);
      // Should include the year, if no volume and year is a prefix
      // Form would be something like: "[\[\(]<year>[\]\)]\s+<reporter>\s+<page>"
      var star = /*options.star ? "([0-9]+)*\*[0-9]+" :*/ "";
      var volume = this.volume ? this.volume + star + "(\\s+)" : "";
      var str = volume + star + rep + star + "(\\s+)" + star + this.page;
      var ret = options.global ? new RegExp(str) : new RegExp(str, g);
  ...
  Citation.prototype.caption = function() {
    return  this.plaintiff && this.defendant ?
              this.plaintiff + ' v. ' + this.defendant :
                this.plaintiff;
  }
  Citation.prototype.full_cite = function() {
    return  this.caption() + ', ' +
            this.base_citation() +
            ' (' + (this.mlz_jurisdiction > 1 && this.court ?
                      this.court :
                        "") +
            this.year + ')';
  }
  ...
  Ferret.constants.FORWARD_SEEK = 20;
  Ferret.constants.BACKWARD_SEEK = 120;
  Ferret.constants.REPORTERS = {
  ...
  Ferret.constants.JURISDICTIONS = {
  ...
  Ferret.constants.ACCEPT_TOKENS = [
      'In Re',
      'In re',
      'Ex parte',
      'Ex Parte'
  ...
  Ferret.constants.STRING_TOKENS = [
      'certiorari denied',
      'cert. denied',
      'denied',
      "aff'd",
  ...
  Ferret.constants.EMBEDDED_TOKENS = [
      "of the",
      "on the",
      "ex rel",
      "et al",
  ...
  Ferret.builders.make_variant_key = function (key) {
      key = key.replace(".", " ", "g")
                .replace(/\s+/g, " ");
      key = " " + key + " ";
  ...
  Ferret.builders.make_variants = function (REPORTERS) {
  ...
  Ferret.builders.make_variants(Ferret.constants.REPORTERS);
  Ferret.builders.suck_out_variations_only = function (REPORTERS) {
      /*
  ...
  Ferret.constants.VARIATIONS_ONLY = Ferret.builders.suck_out_variations_only(Ferret.constants.REPORTERS);
  Ferret.builders.suck_out_courts = function(JURISDICTIONS) {
      var COURTS = {};
  ...
  Ferret.constants.COURTS = Ferret.builders.suck_out_courts(Ferret.constants.JURISDICTIONS);
  Ferret.builders.suck_out_neutrals = function (REPORTERS) {
      /*
  ...
  Ferret.constants.NEUTRALS = Ferret.builders.suck_out_neutrals(Ferret.constants.REPORTERS);
  Ferret.builders.suck_out_editions = function(REPORTERS) {
      /*
  ...
  Ferret.constants.EDITIONS = Ferret.builders.suck_out_editions(Ferret.constants.REPORTERS);
  // We need to build a REGEX that has all the variations and the reporters in_ order from longest to shortest.

  Ferret.builders.make_regex = function (constants) {
      ...
      var REGEX_LIST = _utils.keys(EDITIONS).concat(_utils.keys(VARIATIONS_ONLY));

      REGEX_LIST = REGEX_LIST
          .concat(_utils.values(ACCEPT_TOKENS))
          .concat(_utils.values(EMBEDDED_TOKENS))
          .concat(_utils.values(STRING_TOKENS));
      ...
      var regex_values = _utils.values(REGEX_LIST);
      var REGEX_STR = _utils.map(regex_values, function(i) {
                            return i.replace(".","\\.","g")
                            .replace("(","\\(","g")
                            .replace(")","\\)","g")
                            .replace("\'", "\\'","g");
  ...
  Ferret.builders.make_regex(Ferret.constants);
  Ferret.utils.strip_punct = function (text) {
      //starting quotes
      text = text.replace(/^\"/g, "")
                  .replace(/(``)/g, "")
                  .replace(/([ (\[{<])"/g, '')
  ...
  Ferret.utils.get_visible_text = function (text) {
      return text.replace(/<(?:style|STYLE)[^>]*>.*?<\/(?:style|STYLE)>/g, " ")
                      .replace(/<[Aa] [^>]+>[^ ]+<\/[Aa]>/g, " ")
                      .replace(/<[^>]*>/g, "")
                      .replace("\n"," ","g")
                      .replace("	"," ","g");
  ...
  Ferret.utils.set_jurisdiction = function (citation, jurisdiction) {
      if (!citation.mlz_jurisdiction) {
          citation.mlz_jurisdiction = jurisdiction;
      }
  };

  Ferret.utils.is_date_in_reporter = function (editions, year) {
      /*
  ...
  Ferret.get_court = function (paren_string, year) {
      var court;
      if (!year) {
  ...
  Ferret.get_year = function (token) {
      /*
  ...
  Ferret.get_pre_citation = function (citation, citations, words, reporter_index) {
  ...
  Ferret.carry_forward = function (citations, pos) {
  ...
  Ferret.apply_jurisdiction = function (citation, jurisdiction) {
      if (!citation.mlz_jurisdiction) {
          citation.mlz_jurisdiction = jurisdiction;
      }
  };

  Ferret.apply_year = function (citation, year) {
      if (!citation.year) {
          citation.year = year;
      }
  };

  Ferret.match_jurisdiction = function (citation, data_string) {
  ...
  Ferret._tokenize = function (text) {
  ...
  Ferret.extract_base_citation = function (words, reporter_index) {
      /*
  ...
  Ferret.add_post_citation = function (citation, words, reporter_index) {
      ...
      for (var i=(reporter_index+2),ilen=Math.min((reporter_index+FORWARD_SEEK), words.length);i<ilen;i+=1) {
          ...
          if (words[start].slice(0,1) === "(" || words[start].slice(0,1) === "[") {
              ...
              if (find_pinpoints) {
                  if (words[i].match(/^(?:n\.|n|nn\.|nn|para|para\.|¶|[-0-9]+)[,;]?\s*$/)) {
  ...
  Ferret.add_defendant = function (citations, words, reporter_index) {
      /*
      ...
      var _add_defendant = Ferret.addDefendant(citations, words, pos, idx, end, prev_idx);
      this.buffer = _add_defendant.backscan();
      _add_defendant.finish(citations[pos]);
  }

  Ferret.addDefendant = function (citations, words, pos, idx, end, prev_idx) {
      ...
      return {
          ...
          is_parallel: function() {
              ...
              for (var i=this.idx,ilen=Math.max(this.idx-BACKWARD_SEEK, prev_idx+1, -1);i>ilen;i+=-1) {
                  if (words[i].match(/^(?:n\.|n|para|para\.|¶|[-0-9]+)[,;]?\s*$/)) {
                      idx = i;
                  } else {
                      return false;
                  }
  ...
  Ferret.infer_jurisdiction = function (citations) {
  ...
  Ferret.disambiguate_reporters = function (citations) {
      /*
  ...
  Ferret.get_citations = function (text, options) {
      options = options || { };
      ...
      if ("undefined" === typeof options.html) {
          options.html = true;
      }
      if ("undefined" === typeof options.do_post_citation) {
          options.do_post_citation = true;
      }
      if ("undefined" === typeof options.do_defendant) {
          options.do_defendant = true;
      }
      if (options.html) {
      ...
      var citations = [ ];
      var callback_citations = [ ];

      var KEYS_AND_VARIATIONS = _utils.keys(EDITIONS).concat(_utils.keys(VARIATIONS_ONLY));
      ...
      for (var i=1,ilen=words.length-1;i<ilen;i+=1) {
          // Find reporter
          if (KEYS_AND_VARIATIONS.indexOf(words[i]) > -1) {
              ...
              if(options.update) {
                var rep = words[i];
                callback_citations.push({
                    "citation" : citation,
                    "wordIndex": i,
                    "canonical": citation.canonical_citation(),
                    "regex" : citation.as_regex({star: true, global: true}, VARIATIONS_ONLY[rep]).toString(),
                    "id":options.id || null
                  });
              }

              if (options.do_defendant) {
      ...
      if(options.callback) {
        options.callback(callback_citations);
      }
